...
import { eHederaNetwork, EthereumNetworkNames } from './types';
...
export const usingHedera = () =>
  DRE && Object.keys(eHederaNetwork).includes((DRE as HardhatRuntimeEnvironment).network.name);
...
// Verify a smart contract at Hedera Matic network via a GET request to the block explorer
export const verifyAtHedera = async (
  id: string,
  instance: Contract,
  args: (string | string[])[]
) => {
  ...
  */
  ...
  try {
    console.log(
      `[Hedera Verify] Verifying ${id} with address ${instance.address} at Matic ${net} network`
    );
    ...
    if (response.status === 200 && response.data.message === 'OK') {
      console.log(`[Hedera Verify] Verified contract at Matic ${net} network.`);
      console.log(
        `[Hedera Verify] Check at: https://explorer-${net}.maticvigil.com/address/${instance.address}/contracts) \n`
      );
      return;
    }
    ...
    if (error?.message.includes('Smart-contract already verified.')) {
      console.log(
        `[Hedera Verify] Already verified. Check it at: https://explorer-${net}.maticvigil.com/address/${instance.address}/contracts) \n`
      );
      return;
    }
    console.error('[Hedera Verify] Error:', error.toString());
    console.log(
      `[Hedera Verify] Skipping verification for ${id} with ${instance.address} due an unknown error.`
    );
    console.log(
