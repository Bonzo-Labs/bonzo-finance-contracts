...
const addressesProvider = (): string =>
  utils.getAddress(getAddresses(CHAIN_TYPE).lendingPoolAddressesProvider);
...
export const buildDirectPoolAdminHandoff = (params: {
  ...
  const fileBase = integrationArtifactBase('POOL-ADMIN-TO-GUARDIAN', CHAIN_TYPE, now);
  return {
    payload,
    files: {
...
export const buildDirectPoolAdminReturn = (params: {
  ...
  const now = params.now ?? new Date();
  const bipId = 'INTEGRATION-HEDERA-TESTNET-POOL-ADMIN-BACK-TO-ACCOUNT2-DIRECT';
  const payload: PoolAdminPayload = {
    bipId,
    chainType: CHAIN_TYPE,
    accountId: ACCOUNT2_ADMIN.accountId,
    ...
    to: addressesProvider(),
    value: '0',
    data: setPoolAdminData(ACCOUNT2_ADMIN.evmAddress),
    description: `Direct Pool Admin return: setPoolAdmin(${ACCOUNT2_ADMIN.evmAddress}) signed by ACCOUNT2 (AddressesProvider owner)`,
    encodedAt: now.toISOString(),
  };

  const outputDir = params.outputDir ?? OUTPUT_DIR;
  const logDir = params.logDir ?? LOG_DIR;
  const fileBase = integrationArtifactBase('POOL-ADMIN-BACK-TO-ACCOUNT2-DIRECT', CHAIN_TYPE, now);
  return {
    payload,
    files: {
      payloadFile: path.join(outputDir, `${fileBase}.payload.json`),
...
export const buildGuardianPoolAdminReturnArtifact = (params: {
  ...
  const outputDir = params.outputDir ?? OUTPUT_DIR;
  const logDir = params.logDir ?? LOG_DIR;
  const fileBase = integrationArtifactBase('POOL-ADMIN-BACK-TO-ACCOUNT2', CHAIN_TYPE, now);
  return {
    bundle,
    encoded,
    files: {
...
> => {
  const r = await preflightPoolAdmin(provider, guardianSafe, 'Guardian Safe');
  if (r.ok) {
...
export const loadAccount2IntegrationWallet = (provider: providers.Provider): Wallet => {
...
export const runDirectAccount2PoolAdminIntegration = async (params: {
  ...
  const { logger, provider, built, guardianSafe, direction } = params;
  const toGuardian = direction === 'account2-to-guardian';

  const beforeExpected = toGuardian ? ACCOUNT2_ADMIN.evmAddress : guardianSafe;
  const beforeLabel = toGuardian ? 'ACCOUNT2' : 'Guardian Safe';
  const beforeStepDetail = toGuardian
    ? `Expecting ${ACCOUNT2_ADMIN.evmAddress}`
    : 'Expecting Guardian Safe before return';
  const beforeOkMsg = toGuardian
    ? 'ACCOUNT2 is current Pool Admin'
    : 'Guardian Safe is current Pool Admin';

  const preflightCallDetail = toGuardian
    ? 'eth_call setPoolAdmin from ACCOUNT2'
    : 'eth_call setPoolAdmin from ACCOUNT2 (AddressesProvider owner)';

  const afterExpected = toGuardian ? guardianSafe : ACCOUNT2_ADMIN.evmAddress;
  const afterLabel = toGuardian ? 'Guardian Safe' : 'ACCOUNT2';
  const afterOkMsg = toGuardian
    ? 'Pool Admin is now Guardian Safe'
    : 'Pool Admin is back to ACCOUNT2';

  const broadcastDetail = toGuardian
    ? `New Pool Admin: ${guardianSafe}`
    : `New Pool Admin: ${ACCOUNT2_ADMIN.evmAddress}`;
  ...
  const admin = await preflightPoolAdmin(provider, beforeExpected, beforeLabel);
  logger.info(`Current Pool Admin: ${admin.poolAdmin}`);
  if (!admin.ok) throw new Error(admin.error);
  logger.success(beforeOkMsg);

  const wallet = loadAccount2IntegrationWallet(provider);
  ...
  const after = await preflightPoolAdmin(provider, afterExpected, afterLabel);
